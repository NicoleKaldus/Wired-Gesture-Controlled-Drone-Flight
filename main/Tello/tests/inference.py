...
if __name__ == "__main__":
    ...
    # filePath = f"/data/{true_label}_{num:02d}.csv"
    # print(filePath)
    df = pd.read_csv(f"/Users/nicolekaldus/esp/wirefull-final-project/main/Tello/tests/data/{true_label}_{num:02d}.csv")
    # df = pd.read_csv(f"data/{true_label}_{num:02d}.csv")

    data = df[['acce_x', 'acce_y', 'acce_z', 'gyro_x', 'gyro_y', 'gyro_z']].values.astype(np.float32)

    data = normalize(data)
    model = torch.load("/Users/nicolekaldus/esp/wirefull-final-project/main/Tello/tests/CNN.pth", map_location="cpu") # assuming no cuda
